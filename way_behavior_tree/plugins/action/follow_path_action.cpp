...
bool FollowPathAction::sendGoal(GoalType& msg)
{
...
NodeStatus FollowPathAction::onResult( const ResultType& res)
{
    ROS_INFO("FollowPathAction: result received");
    return NodeStatus::SUCCESS;
}

NodeStatus FollowPathAction::onFailedRequest(FailureCause failure)
{
    ROS_ERROR("FollowPathAction request failed %d", static_cast<int>(failure));
...
void FollowPathAction::halt()
{
    ...
    {
        ROS_WARN("FollowPathAction halted");
        BaseClass::halt();
    }
}
...
{
  RegisterRosAction<way_behavior_tree::FollowPathAction>(factory, "FollowPath");
}